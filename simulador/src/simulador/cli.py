...
def construir_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="simulador",
        description="Simula la lectura remota de medidores agrupados en cabinas sobre un bus RS-485.",
    )
    p.add_argument("--escenario", choices=["A", "B"], default="A",
                   help="A: cabinas con bus compartido (situación actual). "
                        "B: un dispositivo de lectura por medidor, sin contención.")
    ...
    p.add_argument("--semilla", type=int, default=2026,
                   help="Misma semilla, mismos fallos. Es lo que hace reproducibles las pruebas.")
    ...
    p.add_argument("--cada", type=float, default=None, metavar="MIN",
                   help="Minutos entre el arranque de una ronda y la siguiente. "
                        "Por defecto, rondas continuas en el escenario A y cada 15 min en el B, "
                        "que es como operaría un dispositivo dedicado.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    dia = datetime.strptime(args.desde, "%Y-%m-%d").date() if args.desde else datetime.now(tz).date()
    ...
    print(f"período {inicio:%Y-%m-%d %H:%M} → {fin:%Y-%m-%d %H:%M} · semilla {args.semilla} · {ritmo}"
          f"{' · SIN FALLAS' if args.sin_fallas else ''}", file=sys.stderr)

    destino = sys.stdout if args.salida == "-" else open(args.salida, "w", encoding="utf-8")
    n = 0
    try:
        for lectura in generar(parque, cfg, fallas, semilla=args.semilla):
    ...
    print(f"{n} lecturas publicadas" + ("" if args.salida == "-" else f" en {args.salida}"),
          file=sys.stderr)
    return 0
